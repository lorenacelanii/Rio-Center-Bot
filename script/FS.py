...
def alerta_concluido():    
    ...
    tk.messagebox.showinfo("Operação Concluída", "O bot concluiu a operação na FS")
...
for linha in sheet_produtos.iter_rows(min_row=2):
    ...
    pyautogui.sleep(1.5)  # Aguardar um momento para garantir que o texto foi copiado
    ...
    if valor_copiado == codigo:
        ...
        if localizacao is not None:
            ...
            linha[6].value = 'NÃO ALTERADO'
            # Salvar a planilha com as alterações
            workbook.save('ROBO ALT PRECO/BaseAltPreco.xlsx')

        
    else:
        # Clicar em cancelar
        pyautogui.doubleClick(1212,150, duration=1)
        linha[6].value = 'CÓDIGO ERRADO'
        # Salvar a planilha com as alterações
        workbook.save('BaseAltPreco.xlsx')
